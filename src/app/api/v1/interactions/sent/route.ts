...
export async function GET(request: Request) {
  try {
    ...
    // Fetch profiles I have expressed interest in (user_id = userId, action = 'interested')
    // AND we are not already matched (that belongs in the Matched tab)
    // AND they have not declined me (a dead pending state, not worth showing as "sent")
    const query = sql`
      SELECT
      ...
      WHERE i.user_id = ${userId}
        ...
        AND NOT EXISTS (
          SELECT 1 FROM interactions their_i
          WHERE their_i.user_id = u.id
            AND their_i.target_id = ${userId}
            AND their_i.action = 'declined'
        )
        AND NOT EXISTS (
